...
export class AnalyticsService {
  ...
  placement(email):Observable<any>
  {
    let url = `${this.baseurl}details/${email}`;
    console.log(url)
    return this.http.get(url);
  }
  getSubjects(year,term,id):Observable<any>{
    let url=`${this.baseurl}faculties/subjects/${year}/${term}/${id}`;
    console.log(url)
  ...
  getInternal(year,subname,empid):Observable<any>{
    let url = `${this.baseurl}${year}/${subname}/${empid}`;   
    console.log("hod",url)
    return this.http.get(url);
  }
}